...
$parentModule = 'MsgToEml'
if (-not $ModuleName) { $ModuleName = 'MsgToEml.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MsgToEml'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MsgToEml.psd1"
$prereqHash = @{
